                ...
                <ul class="list-unstyled mb-0">
                    @foreach($topScores as $entry)
                        <li>
                            ...
                            @endif
                            <strong>{{ $entry->user?->name ?? __('socialprofile::messages.leaderboards.unknown') }}</strong> — {{ $entry->score }}
                        </li>
                    @endforeach
                </ul>
            </div>
                    ...
                    @foreach($topActivity as $entry)
                        <li>
                            @if($loop->first)
                                <span class="me-2">🥇</span>
                            @endif
                            <strong>{{ $entry->user?->name ?? __('socialprofile::messages.leaderboards.unknown') }}</strong> — {{ $entry->points }}
                        </li>
                    @endforeach
                </ul>
            </div>
