...
Ноябрьский релиз принёс полноценную систему прогрессии:

- **Вкладка админки.** Новый раздел «Уровни прогрессии» позволяет управлять рейтингами, порогами и действиями. Действия на порогах могут выдавать роли Azuriom, переключать функции плагина, отмечать виртуальные права, вызывать вебхуки или устанавливать внутренние флаги.
- **Движок правил.** Правила слушают внутренние события (`activity.updated`, `score.updated`, `coins.updated`, `trust.updated`, `violation.added` и др.) и применяют настраиваемые дельты с кулдаунами и условиями на полях.
- **Виджет профиля.** В профиле игрока социальный и активностный рейтинги отображаются в виде прогресс-баров с маркерами достижений и подсказками.
- **Открытое API.** Эндпоинты `/api/social/v1/progression/ratings`, `/api/social/v1/user/{nickname}/progression` и `/api/social/v1/user/{nickname}/progression/events` отдают метаданные, текущие значения и принимают события (scope `progression:read` / `progression:write`).
- **Готовый сценарий.** По умолчанию социальный рейтинг настроен как в описанном сценарии (бан при `< 0`, статусы для диапазонов, доп. приваты, баллы поддержки >1000). Любой порог/действие можно отредактировать из панели админа без правки кода.
